...
describe("Notes", function()
    ...
    describe("assertAnchor", function()
        it("accepts every kind with its required fields", function()
            ...
            Notes.assertAnchor({ kind = "scratchpad" })
        end)

        it("knows which kinds have a place in the book", function()
            assert.is_false(Notes.hasLocation("book"))
            assert.is_false(Notes.hasLocation("scratchpad"))
            assert.is_true(Notes.hasLocation("page"))
            assert.is_true(Notes.hasLocation("chapter"))
            assert.is_true(Notes.hasLocation("highlight"))
            assert.has_error(function() Notes.hasLocation("paragraph") end)
        end)
    ...
    describe("sameAnchor", function()
        ...
        it("treats every scratchpad anchor as the same note, distinct from the book note", function()
            assert.is_true(Notes.sameAnchor({ kind = "scratchpad" }, { kind = "scratchpad" }))
            assert.is_false(Notes.sameAnchor({ kind = "scratchpad" }, { kind = "book" }))
        end)
    ...
    describe("toSaveable / fromSaved", function()
        ...
        it("round-trips a scratchpad store", function()
            local store = Notes.newStore()
            local pad = Notes.add(store, Notes.newNote({ kind = "scratchpad" }, 7))
            table.insert(pad.pages[1].strokes, stroke(5, 6))
            local loaded = Notes.fromSaved(Notes.toSaveable(store, strip), restore)
            assert.equals(1, #loaded.notes)
            assert.same({ kind = "scratchpad" }, loaded.notes[1].anchor)
            assert.equals(5, loaded.notes[1].pages[1].strokes[1].points[1].x)
            assert.is_not_nil(Notes.find(loaded, { kind = "scratchpad" }))
            assert.is_nil(Notes.find(loaded, { kind = "book" }))
        end)
    ...
    describe("browseOrder", function()
        ...
        it("puts the scratchpad before the book note", function()
            local store = Notes.newStore()
            local p1 = Notes.add(store, note({ kind = "page", page = 1 }))
            local book = Notes.add(store, note({ kind = "book" }))
            local pad = Notes.add(store, note({ kind = "scratchpad" }))
            assert.same({ pad, book, p1 }, Notes.browseOrder(store, stored))
        end)

        it("never asks for the location of the book note or the scratchpad and checks the locator", function()
            local store = Notes.newStore()
            Notes.add(store, note({ kind = "book" }))
            Notes.add(store, note({ kind = "scratchpad" }))
